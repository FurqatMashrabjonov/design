...
5. Icons: `<i data-lucide="name"></i>`, coloured with `currentColor` (see the icons and fonts contract). Never hand-draw icon SVG. Images: write `<img data-od-img="what the photo shows, 3-6 plain words" alt="…">` with **no `src`** — a matching stock photo is found and inserted for you. Give the image its box in CSS (`width`, plus `aspect-ratio` or `height`); it is cropped to fit with `object-fit: cover`. Never invent an image URL and never use a placeholder service. People's avatars are initials in a circle, not photos.
