...
**Charts.** Never draw a chart yourself. Write an empty slot with the data and it is drawn for you in the design system's colours: \`<div data-od-chart="bar" data-values="4,5,3,6" data-labels="M,T,W,T" data-highlight="3"></div>\`. Types: \`bar\`, \`line\`, \`area\`, \`sparkline\` (tiny trend in a row or card), \`donut\` (shares, with \`data-labels\` as the legend), \`ring\` (progress: \`data-values="7.2" data-max="10" data-unit="k" data-labels="steps"\`), \`heatmap\` (a streak calendar: one value per day, \`data-columns="7"\`). Values match the numbers shown on the screen. Size with a style height if needed; titles and totals go outside the slot.
...
**Stickers.** No emoji. For one piece of character (a flame on a streak, a trophy) write \`<div data-od-sticker="fire"></div>\` — drawn as a soft-3D sticker in the system's colours. Names: fire, trophy, leaf, star, moon, drop, bolt, heart, target, check, sparkle, calendar. Style width sets the size; \`data-tone="success|warn|danger"\`. One or two per screen.
