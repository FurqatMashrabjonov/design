...
export const EditPairService = {
  async forProject(projectId: string): Promise<EditPair[]> {
    const project = await Project.find(projectId)
    if (!project) return []
    const messages = await Message.forProject(projectId)
    const pairs: EditPair[] = []
    for (const [i, m] of messages.entries()) {
      if (m.role !== 'agent' || !CHANGES.has(m.kind)) continue
      ...
      for (const ref of meta.screens ?? []) {
        ...
        const screen = await Screen.find(ref.id)
        const before = screen && await ScreenVersion.findInScreen(ref.versionId, screen.id)
        if (!screen || !before) continue
        const ids = await ScreenVersion.ids(screen.id)
        const nextId = ids[ids.indexOf(before.id) + 1]
        const after = nextId ? (await ScreenVersion.findInScreen(nextId, screen.id))!.html : screen.html
