...
const marginColumns: Column<Margin>[] = [
  ...
  { key: 'margin', header: 'Margin', cell: (m) => <span className={m.margin < 0 ? 'text-red-600' : ''}>{m.margin < 0 ? `−${money(-m.margin)}` : money(m.margin)}</span>, sort: (m) => m.margin, className: 'text-right tabular-nums' },
]
    ...
    <>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <Panel title="Revenue · 30 days" className="lg:col-span-2">
          <DailyChart rows={d.byDay} series={[{ key: 'revenue', label: 'Revenue', color: '#16a34a' }]} format={(n) => `$${Math.round(n)}`} />
        </Panel>
