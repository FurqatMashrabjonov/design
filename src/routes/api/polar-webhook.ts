  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const ok = verify((await SecretService.get('POLAR_WEBHOOK_SECRET')) ?? '', { id: h.get('webhook-id'), timestamp: h.get('webhook-timestamp'), signature: h.get('webhook-signature') }, body)
