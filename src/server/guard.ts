...
export async function guardGeneration(request: Request, run: (req: Request, userId: string) => Promise<Response>): Promise<Response> {
  ...
  let res: Response
  try {
    res = await UsageService.run(who, () => run(new Request(request.url, { method: 'POST', headers: request.headers, body: text, signal: request.signal }), user.id))
  } catch (e) {
