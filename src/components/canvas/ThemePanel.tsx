...
import { FONTS, RADIUS_OPTIONS, isEmptyTheme, type Radius, type Theme } from '@/lib/theme-override'
    ...
    <div className="space-y-5 text-sm">
      ...
      <section className="space-y-2">
        ...
        <div className="grid grid-cols-4 gap-1 rounded-lg bg-muted p-1">
          {[{ id: undefined, label: 'Default' }, ...RADIUS_OPTIONS].map((o) => (
            <button
              key={o.label}
              type="button"
              onClick={() => set({ radius: o.id as Radius | undefined })}
              className={cn(
                'h-8 rounded-md text-xs transition-colors',
                theme.radius === o.id ? 'bg-background font-medium shadow-sm' : 'text-muted-foreground hover:text-foreground',
              )}
            >
              {o.label}
            </button>
