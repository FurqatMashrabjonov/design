...
- **Charts are slots.** The model writes `<div data-od-chart="bar|line|area|sparkline|donut|ring|heatmap" data-values="…" data-labels="…">` and `lib/charts.ts` (called from `autofixScreen`) draws it from the data with token colours. The model never draws a chart; `charts.handDrawnScreens` in the eval counts the ones it did.
