...
- **The render audit sees what code cannot** (`lib/render-audit.ts`): it runs inside each frame after it settles and reports overflow, clipped text, small targets, low contrast and overlaps with the offending element's `data-od-id`. Findings from the frame are untrusted and go through `parseAudit`. `eval/audit.ts <run>` measures a run the same way in headless Chrome.
...
- **Every model call is logged** (`llm_calls`, via `LlmService.onLlmCall` + `UsageService`), attributed to the request's user and project through `AsyncLocalStorage`. The limits (`UsageService.refusal`: calls per user per day, one generation at a time, daily budget, `GENERATION_PAUSED`) read that table. A new generation path must run inside `guardGeneration` or it escapes both.
